...
# -*- coding: utf-8 -*-
"""
@file   get_hand_result.py
@brief  Get hand result example.
@details This example runs hand detection and displays results (bounding box and keypoints) on a tkinter canvas.
@n      Uses MODEL_HAND; detection runs in a background thread and updates the GUI.
...
def print_hand_keypoints(r):
  """Print all 21 hand keypoints (same order as driver / canvas)."""
  if not isinstance(r, HandResult):
    return
  for attr in HAND_KEYPOINT_ATTRS:
...
if __name__ == "__main__":
  ...
  if not humanpose.begin():
    ...
    humanpose.set_model_type(humanpose.MODEL_HAND)
